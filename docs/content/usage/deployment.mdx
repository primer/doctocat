...
To fix this issue, you need to define a couple `routes` in your [`now.json`](https://zeit.co/docs/v2/advanced/routes). You'll need to define a route that proxies `/my-site/*` to `/*` so that behind the scenes a path like `/my-site/getting-started` will be treated as `/getting-started`. You'll also want to define a route that redirects traffic from `/` to `/my-site` so that if a user goes to `/` they don't get a 404 error. Here's what your `now.json` should look like:
...
You'll able to access your site from `primer.style/my-site` once your pull request is approved and merged.
